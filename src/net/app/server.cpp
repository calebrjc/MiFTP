#include "calebrjc/net/app/server.hpp"
...
server::server(uint16_t port)
    : port_(port),
      poll_group_(detail::poll::socket_status::readable),
      running_(false),
...
void server::handle_incoming_messages_() {
    detail::poll::poll_result events = poll_group_.poll();
    ...
    for (const auto &[socket_fd, status] : events) {
        ...
        using namespace calebrjc::net::detail::poll;

        if ((status & socket_status::error) || (status & socket_status::hung_up)) {
        ...
        if (status & socket_status::readable) {
